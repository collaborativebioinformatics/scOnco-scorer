...
# --- 3b. log loss (METHODS.md section 8 requires it alongside Brier) ---------
y <- c(1L, 0L, 1L, 0L)
check("log_loss: perfect prediction = 0", log_loss(y, c(1,0,1,0)) < 1e-9)
check("log_loss: confident and wrong is finite, not Inf",
      is.finite(log_loss(c(1L,0L), c(0,1))),
      sprintf("value = %.1f (clamped at eps)", log_loss(c(1L,0L), c(0,1))))
check("log_loss: always-0.5 equals log(2)",
      abs(log_loss(y, rep(0.5, 4)) - log(2)) < 1e-9)
check("log_loss punishes overconfidence harder than Brier",
      { a <- c(0.6,0.4,0.6,0.4); b <- c(0.99,0.01,0.99,0.01)
        yy <- c(1L,0L,0L,1L)          # two right, two badly wrong
        (log_loss(yy,b)/log_loss(yy,a)) > (brier(yy,b)/brier(yy,a)) })
